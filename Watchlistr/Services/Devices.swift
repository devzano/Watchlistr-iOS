...
// #-#-#-#-#-#-#-#-#-#-#-#-#
    ...
    var type: Model {
        ...
        if let mcode = modelCode, let model = modelMap[mcode] {
            return model
        }

        if let simModelCode = ProcessInfo.processInfo.environment["SIMULATOR_MODEL_IDENTIFIER"] {
            return modelMap[simModelCode] ?? .simulator
        }
...
extension Model {
    static let plusAndMaxModels: [Model] = [
    ...
    static let nonPlusNonMaxModels: [Model] = [
        .iPhone4, .iPhone4S,
